...
#include <ant/utils/FilePathContainer.h>

GameState::GameState(ant::IApp& theApp) :
  ant::IState("Game",theApp),
...
void GameState::init()
{
  ...
  IState::init();
	...
	ant::IApp::getApp()->mWindow.setIcon(32,32,icon.getPixelsPtr());

	//Load the area
	antUtil::FilePathContainer fp;
	fp.add(RESOURCE_DIR);
