...
namespace ant
{
		...
		mScopeName(scopeName)
	{
		...
		if(ant::ILogger::getLogger())
		{
			...
			ant::ILogger::getLogger()->getStream(mSeverity, mSourceFile, mSourceLine,
	...
	ScopeLogger::~ScopeLogger()
	{
		...
		if(ant::SeverityFatal == mSeverity)
		{
			fatalShutdown();
		}
	}
	...
	std::ostream& ScopeLogger::getStream()
	{
		...
		if(ant::ILogger::getLogger())
		{
			...
			return ant::ILogger::getLogger()->getStream();
		}
		else
		{
			// If global logger doesn't exist, gNullStream will so return that
			return ant::ILogger::gNullStream;
		}
	}
	...
	std::ostream& ScopeLogger::getStream( SeverityType severity, const std::string& sourceFile, int sourceLine, int exitCode /*= StatusError*/ )
	{
		...
		{
			// Just return whatever the global logger returns for GetStream()
			return ant::ILogger::getLogger()->getStream(severity, sourceFile,
	...
	void ScopeLogger::logMessage( const std::string& message )
	{
		...
		if(ant::ILogger::getLogger())
		{
			...
			ant::ILogger::getLogger()->logMessage(message);
		}
	}
...
;
		...
		if(ant::ILogger::getLogger())
		{
			...
			ant::ILogger::getLogger()->logMessage(severity, sourceFile, sourceLine,
				message);
		}
	}
}